...
// savedDiagnosticContext holds the diagnostic state that setupDiagnosticContext
// overwrites. Returning it by value rather than as a restore closure keeps this
// off the heap; setupDiagnosticContext runs once per visited node, so a closure
// here is one of the largest allocation sources in declaration emit.
type savedDiagnosticContext struct {
	diag                          GetSymbolAccessibilityDiagnostic
...
func (tx *DeclarationTransformer) setupDiagnosticContext(input *ast.Node) (bool, savedDiagnosticContext) {
	saved := savedDiagnosticContext{
		diag:                          tx.state.getSymbolAccessibilityDiagnostic,
	...
	return canProdiceDiagnostic, saved
}

func (tx *DeclarationTransformer) restoreDiagnosticContext(saved savedDiagnosticContext) {
	tx.state.getSymbolAccessibilityDiagnostic = saved.diag
...
func (tx *DeclarationTransformer) visitDeclarationSubtree(input *ast.Node) *ast.Node {
	...
	canProdiceDiagnostic, saved := tx.setupDiagnosticContext(input)
	defer tx.restoreDiagnosticContext(saved)
...
func (tx *DeclarationTransformer) visitCJSExportAssignments(expression *ast.Node) *ast.Node {
	if expression != nil {
		_, saved := tx.setupDiagnosticContext(expression)
		defer tx.restoreDiagnosticContext(saved)
...
func (tx *DeclarationTransformer) transformExpandoAssignment(node *ast.BinaryExpression) {
	...
	_, saved := tx.setupDiagnosticContext(node.AsNode())
	defer tx.restoreDiagnosticContext(saved)
...
func (tx *DeclarationTransformer) transformExpandoHost(name *ast.Node, declaration *ast.Declaration) {
	...
	_, saved := tx.setupDiagnosticContext(declaration)
	defer tx.restoreDiagnosticContext(saved)
